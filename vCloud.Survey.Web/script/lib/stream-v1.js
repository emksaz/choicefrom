...
(function(){
	var Provider, aFilters = [], nIdCount = 0, aOtherBrowsers = ["Maxthon", "SE 2.X", "QQBrowser"],
		...
		sCellFileTemplate = '<b class="stream-uploading-ico"></b>' +
					...
					'	<span class="stream-cell-info"><em class="stream-speed"></em></span>' +
					'	<span class="stream-cell-info"><em class="stream-uploaded"></em></span>' +
					'	<span class="stream-cell-info"><em class="stream-remain-time"></em></span>' +
					'</div>',
		sTotalContainer = '<div id="#totalContainerId#" class="stream-total-tips">' +
			'<span class="stream-process-bar"><span style="width: 0%;"></span></span>' +
			'<span class="stream-percent">0%</span><strong class="_stream-total-uploaded">&nbsp;</strong>' +
	...
	StreamProvider.prototype = {
		...
		renderUI : function(a) {
			this.contentBox = a;
			this.fileInputField = fCreateContentEle("<input type='file' id='btn_Upload' style='visibility:hidden;width:0px;height:0px;'>");
	...
	StreamUploader.prototype = {
		...
		streamUpload: function(pos){
			...
			//console.log("I am is streamUpload");
			...
			var blob = this.sliceFile(this.file, pos, pos + this.EveryTimeStream);
		...
		uploadEventHandler: function(event){
			//console.log("I am is uploadEventHandler");
			...
			switch(event.type){
				case "load":
					...
					if(uploaded < this.get("size") -1) {
						...
						//console.log(this.get("uploadURL"));
						this.fire("uploadcomplete", {originEvent : event, data : event.target.responseText,token:this.EveryToken});
					}
					break;
				case "error":
				...
				case "progress":
					...
					if (0.68 <= cost || 0 === this.bytesSpeeds.length) {
						...
						this.bytesSpeeds.push(this.bytesSpeed);
						//console.log("I am is progress");
						//console.log(this.bytesSpeeds);
					...
					//console.log(this.bytesPrevLoaded);
					this.fire("uploadprogress", {
								originEvent : event,
								bytesLoaded : loaded,
								bytesTotal : total,
		...
		startUpload: function(url, postVars, fileFieldName){
			this.EveryToken=postVars.token;
			this.fileStartPosValue = null;
			this.retriedTimes = 0;

			postVars.name = this.get("name");
			postVars.size = this.get("size");
			//console.log(postVars.size);
			...
			switch (method) {
				case "formUpload" :
					...
					break;
				case "streamUpload" :
					conso.log("I Will into streamUpload2");
					this.streamUpload();
					break;
				case "resumeUpload" :
					this.resumeUpload()
	...
	Main.prototype = {
		...
		addStreamTask : function(a) {
			...
			if (!this.config.customered) {
				//console.log("I am is addStreamTask-this.config");
		...
		upload : function(index) {
			...
			if(this.uploading) return;//false false
			index = this.waiting.shift();
			//console.log(index);//false
			if(index == null) return;//file_1417061882072_01v_3  undefine
			this.uploading = !0;
			////console.log("wen");//false
			this.uploadInfo[index].actived = !0;
			var file = this.uploadInfo[index].file, self = this;
			//console.log("I am file start");
			//console.log(file);
			//console.log("I am file end");
			var frmUploadURL = this.get("frmUploadURL");
			var uploadURL = this.get("uploadURL");
			////console.log(frmUploadURL);//php/upload.php?action=fd; 
			////console.log(uploadURL);//php/upload.php?action=up 
			...
			//php/upload.php?action=tk&
			//name=4.jpg
			//type=image%2Fjpeg
			//size=15569//
			//modified=Mon+Nov+17+2014+10%3A47%3A22+GMT%2B0800+(%E4%B8%AD%E5%9B%BD%E6%A0%87%E5%87%86%E6%97%B6%E9%97%B4)&38859 
			//console.log("xhr1");
			//console.log(tokenUrl);
			...
			xhr.onreadystatechange = function() {
				...
				try {
					...
					if (token) {
						...
						//console.log("I will into uploadFile");
						//console.log(file);
		...
		uploadFile : function(file, url, token, method) {
			//console.log("I am is uploadFile");
			//console.log(file);
			//console.log(url);
			//console.log(token);
			//console.log(method);
		...
		uploadProgress : function(a) {
			...
			var percent = (bytesLoaded) * 10000 / (this.totalFileSize-this.totalUploadedSize) / 100, percent = Math.min(99.99, percent);
			...
			this.getNode("stream-process-bar", this.totalContainerPanel).innerHTML = '<span style="width: '+percent+'%;"></span>';
			this.get("onUploadProgress") ? this.get("onUploadProgress")(percent) : this.onUploadProgress(percent);
		},
		uploadComplete : function(a) {
			...
			var info = {
				...
				totalPercent:       totalPercent,
				msg:                a.data,
				token:a.token+a.target.get("name").substring(a.target.get("name").lastIndexOf("."))
			};
		...
		fileSelect : function(a) {
			...
			//console.log("I am is fileSelect");
			//console.log(files);
			...
			//console.log("I am is fileSelect2");
			//console.log(files);
		},
